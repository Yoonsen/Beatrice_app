...
@st.cache_data()
def concordance(
        dataframe, words = None, window = 25, limit = 100
):
    ...
    if words is None:
        ...
        res = dh.Concordance(dataframe.urn, query=words, window=window, limit=limit).frame
...
if konk_ord != '':
    concs = concordance(st.session_state['korpus'], words=konk_ord, limit=antall, window=kontekst)[['urn','concordance']]
    #st.write(concs)
    concs['url'] = concs.urn.apply(lambda x: f"""https://www.nb.no/items/{x}?searchText="{konk_ord}" """)
    concs['Konkordans'] = concs.concordance.apply(lambda x: x.replace('<b>','**').replace('</b>', '**'))
